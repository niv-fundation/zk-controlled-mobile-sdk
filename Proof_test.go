...
func TestEncodeIdentityProof(t *testing.T) {
	...
	expectedData, err := hex.DecodeString(expectedHex[2:])
	if err != nil {
		t.Fatalf("Error decoding expected hex: %v", err)
	}

	// Compare the encoded data
	if !bytes.Equal(encodedData, expectedData) {
		t.Errorf("Encoded data does not match expected output.\nGot:      %x\nExpected: %x", encodedData, expectedData)
	}
}
